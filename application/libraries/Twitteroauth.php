...
class TwitterOAuth {
    ...
    public function testMediaUpload1()
    {
        $path = './uploads/home/565ff161239ca_thumb.jpg';
        ...
        $data = $this->oAuthRequest($url, $method, $params);
        $expected = 'image\/png';
        //$test->assertContains($expected, $data);
        /** Store the media id for later **/
        $data = @json_decode($data, true);
        //$test->assertArrayHasKey('media_id', is_array($data) ? $data : array());
        return $data['media_id'];
    }
    public function testMediaUpload($path)
    {
        $file = file_get_contents($path);
        $data = base64_encode($file);
        $url    = 'https://upload.twitter.com/1.1/media/upload.json';
