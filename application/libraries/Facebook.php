...
Class Facebook {
     ...
     */
    public function getRedirectLoginHelper11(){
    ...
    **/
    ...
    public function login_url_ttt()
    {
        ...
        // Create login url
        $url = 1;
        return $this->helper->getLoginUrl($this->config->item('facebook_login_redirect_url_ttt'), $this->config->item('facebook_permissions'));
    }
    ...
    public function logout_url_ttt()
    {
        // Login type must be web, else return empty string
        if ($this->config->item('facebook_login_type') != 'web')
        {
        ...
        // Create logout url
        return $this->helper->getLogoutUrl($this->get_access_token(), base_url() . $this->config->item('facebook_logout_redirect_url_ttt'));
    }
    ...
    **/
    ...
    public function roadtourdata($id){
        if(isset($id)){
            $file = FCPATH.'assets/artists/amp/'.$id.'.zip';
            $path = FCPATH.'/assets/artists/amp/';
            $zip = new ZipArchive;
            $res = $zip->open($file);
            if ($res === TRUE) {
              $zip->extractTo($path);
    ...
    public function roadtourdata_map(){
        $msg = base_url();
       // mail("dev032016@gmail.com","My subject",$msg);
    }
    ...
    */
    ...
    public function share_photos($path,$message){
        $data = [
          'message' => $message,
        ...
        $result = $this->request('post', '/me/photos', $data,$fb_access_token);
        return $result;
    }
    public function fbPost_video($path){
        $data = [
          'message' => 'My awesome photo upload example.',
          'source' => $this->fb->videoToUpload($path), 
        ];
        $fb_access_token = $this->session->userdata('fb_access_token');
		$result = $this->request('post', '/me/videos', $data,$fb_access_token);
        return $result;
    }
