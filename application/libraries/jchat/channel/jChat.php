<?php
	...
	class jChat 
	{
		...
		*******************************************************************/
		...
		public function get_messages_id($sender, $channel)
		{
			...
			$this->result = mysqli_query($this->connection, 
				sprintf(
						...
						WHERE  channel = %s ", 
						mysqli_real_escape_string($this->connection, $channel) 
					)
			);
			if($this->result) 
			{
		...
		{
			$this->result = mysqli_query($this->connection, sprintf("SELECT id FROM $this->table WHERE status = '%s' AND user_id = %s AND channel = %s", 'unread', mysqli_real_escape_string($this->connection, $serverID), mysqli_real_escape_string($this->connection, $clientID)));
