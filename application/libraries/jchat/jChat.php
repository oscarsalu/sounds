	...
	**************************************************************************/
	...
	class jChat 
	{
		...
		*******************************************************************/
		...
		public function get_messages_id($sender, $receiver)
		{
			...
			$this->result = mysqli_query($this->connection, 
				sprintf(
						...
						mysqli_real_escape_string($this->connection, $sender), 
						mysqli_real_escape_string($this->connection, $receiver),
						mysqli_real_escape_string($this->connection, $receiver), 
						mysqli_real_escape_string($this->connection, $sender)
					)
			);
		...
		{
			$this->result = mysqli_query($this->connection,sprintf("SELECT id FROM $this->table WHERE status = '%s' AND user_id = %s AND receiver = %s", 'unread', mysqli_real_escape_string($this->connection, $serverID), mysqli_real_escape_string($this->connection, $clientID)));
			...
			$this->result = mysqli_query($this->connection,
				sprintf("INSERT INTO $this->table
						...
						", 
						 ...
						 mysqli_real_escape_string($this->connection, $this->clientID),
						 mysqli_real_escape_string($this->connection, $this->serverID),
						 mysqli_real_escape_string($this->connection, $attachment)
				)
			);
		...
		public function set_user_sessionStatus($clientID, $status)
		{
			...
			switch($status)
			{
				case "ONLINE":
					...
					if($is_online['session'] == 'offline' || $is_online['session'] == NULL) // not online set online
					{
						$this->result = mysqli_query($this->connection, 
