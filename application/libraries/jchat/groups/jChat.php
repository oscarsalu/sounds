<?php
	...
	class jChat 
	{
		...
		*******************************************************************/
		...
		//////////////////////////
		public function get_messages_id($sender, $groups)
		{
			// Result Query
			$this->result = mysqli_query($this->connection, 
				sprintf(
						"SELECT id, user_id, group_id, storage_a, storage_b FROM $this->table
						WHERE  group_id = %s ", 
						mysqli_real_escape_string($this->connection, $groups) 
					)
			);
			if($this->result) 
			{
		...
		{
			$this->result = mysqli_query($this->connection, sprintf("SELECT id FROM $this->table WHERE status = '%s' AND user_id = %s AND group_id = %s", 'unread', mysqli_real_escape_string($this->connection, $serverID), mysqli_real_escape_string($this->connection, $clientID)));
