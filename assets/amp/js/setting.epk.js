...
function load_data_options(playlistAlbumIds ,playlistUserId ,affiliateId,user_id ,background_color ,color_track_front ,color_font ,color_time_loaded ,orderText ,affiliatetext) {
...
function AMP_Load_data(user_id, affiliateId, playlistAlbumIds) {
   if (Loaded_script) {
        // AMP_Load_script();     
        $.ajax({
         type: "GET",
         url: DOMAIN + "map/get_option_widget_user_id?user_id="+user_id ,
         dataType: "text",
        ...
        setTimeout("AMP_Load_data('"+user_id+"','"+affiliateId+"','"+playlistAlbumIds+"')", 50);  
    }
