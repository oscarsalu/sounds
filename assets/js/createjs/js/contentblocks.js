...
//
//var restUrl = 'http://localhost:7070/sound_/find?q={"@subject": "[id]"}"';
//var url_c =  /*'http://localhost:7070/sound_/' */ BASE_URL;//
...
Backbone.sync = function (method, model, options) {
    ...
    if (method == 'create') { 
        ...
        //alert(url_c+'change_text');      
        $.ajax({                        
            type: 'post',
            dataType: 'json',   
            url: BASE_URL+'change_text',                                 
            data: {html_text:JSON.stringify(model),id:CommonManager.encodeId(model.id)},                     
            success: function(data){  
                alert(data);
                if(data = 'success'){
                    location.reload();    
