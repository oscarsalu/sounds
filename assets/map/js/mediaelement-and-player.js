...
mejs.HtmlMediaElementShim = {
	...
	createErrorMessage: function(playback, options, poster) {
		...
		if (!errorContent) {
			...
			errorContent += '<span>' + mejs.i18n.t('Don\'t have songs') + '</span></a>';
		}
	...
	mejs.MediaElementPlayer.prototype = {
                ...
                for (var i = 0; i < document.playlists.length; i++) {
            		if (document.playlists[i].albums) {
            			...
            			if (document.playlists[i].albumId == document.currentAlbumId) {
            				img_cover = document.playlists[i].albums[0].img_cover
            			}
            		}
            	}
                if(typeof(img_cover) == "undefined" || img_cover == null ){
                    img_cover = 'http://lebel.tech/assets/images/logo_sound99.png'
                }
				// build container
				t.container =
					$('<div id="' + t.id + '" class="mejs-container ' + (mejs.MediaFeatures.svg ? 'svg' : 'no-svg') +
                      '" tabindex="0" role="application" aria-label="' + videoPlayerTitle + '" >'+
                        '<div class="bgCont">'+
                            '<div class="bg" style="background: url(&quot;'+img_cover+'&quot;) 50% 50% / cover;">'+
						...
						'<div class="mejs-inner">'+
                            ...
                            '<div class="mejs-image-cover"><img src='+img_cover+' height="55" width="55"/></div>'+
			...
			if (!(mf.isAndroid && t.options.AndroidUseNativeControls) && !(mf.isiPad && t.options.iPadUseNativeControls) && !(mf.isiPhone && t.options.iPhoneUseNativeControls)) {
				...
				media.addEventListener('play', function () {
				    ...
				    if (document.currentSong) {
				        ...
				        var song = document.currentSong;

				        // track name ticker

				        var trackInfoText = song.title;
				        if (song.artist && song.artist.length > 0) {
				            trackInfoText = trackInfoText + " - Artist: " + song.artist;
				        }
				        if (song.albumName && song.albumName.length > 0) {
				            trackInfoText = trackInfoText + " - Album: " + song.albumName;
				        }

				        var trackInfoHtml = "<div class = 'marquee'>" + trackInfoText + "</div>";
		...
		setPlayerSize: function(width,height) {
			...
			if (t.height.toString().indexOf('%') > 0 || (t.$node.css('max-width') !== 'none' && t.$node.css('max-width') !== 't.width') || (t.$node[0].currentStyle && t.$node[0].currentStyle.maxWidth === '100%')) {
				...
				if (t.options.playerHeight) {

				    t.layers.children('.mejs-layer').width(t.width);

				    $.each(t.layers.children('.mejs-layer'), function (key, value) {

				        var thisLayer = $(t.layers.children('.mejs-layer').get(key));

				        if (thisLayer.hasClass("mejs-playlist")) {
				            thisLayer.height(t.options.playerHeight - 24);
				        } else {
				            thisLayer.height(t.height);
				        }
		...
		setControlsSize: function() {
			...
			if (railWidth === 0 || !railWidth) {
				...
				others.each(function() {
					var $this = $(this);
					if ($this.css('position') != 'relative' && $this.is(':visible')) {
			...
			do {
				...
				if (lastControl.css('position') != 'relative') {
		...
		setProgressRail: function(e) {
			...
			if (percent !== null) {
				...
				if (t.loaded && t.total) {
                    ...
                    t.loaded.css("background", document.color_time_loaded);
				}
			}
		},
